...
@Component
public class TmsCteInsuranceValidation {

	@Autowired
	CustomerInfoRepo repo;

	public StringBuilder cteInsuranceValidation(TmsCteInsuranceMapper tmscte) {
		...
		if (value != null) {
			concat(content, "2", "#"); // cliente
			concat(content, tmscte.getPayerTaxId().replaceAll("[()?:!/.,;{}-]", ""), "#");// changed to custtaxid

		} else {
			concat(content, "1", "#"); // fedex
			concat(content, tmscte.getCnpjRespForInsurance().replaceAll("[()?:!/.,;{}-]", ""), "#");// changed to
																									// custtaxid

		}

		concat(content, tmscte.getInsurerName(), "#");// exchanged
		concat(content, tmscte.getCnpjInsurance().replaceAll("[()?:!/.,;{}-]", ""), "#"); // Insurance company Cnpj
